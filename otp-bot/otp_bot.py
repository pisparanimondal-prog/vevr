...
def send_telegram_message(config, number, service, raw_text, otp_code, api_name):
    ...
    inline_keyboard = {
        'inline_keyboard': [
            [{**{'text': '• • • • • •', 'copy_text': {'text': str(otp_code)}}, **_s('copy')}],
            [
                {**{'text': '🤖 Bot Link', 'url': config.get('settings', {}).get('bot_link_url', 'https://t.me/YourBot')}, **_s('bot_link')},
                {**{'text': '📢 Channel',  'url': config.get('settings', {}).get('channel_link_url', 'https://t.me/YourChannel')}, **_s('channel')},
            ],
        ]
    }
...
def api_worker(api_id):
    ...
    while _running:
        try:
            ...
            if is_conn_error:
                logger.warning('Connection error detected — recreating session')
                try:
                    session.close()
                except Exception:
                    pass
            ...
            if retry_after > 0:
                ...
                display_err = f'Server unreachable (connection dropped) — retry #{consecutive_failures}'
            ...
            if retry_after > 0:
                ...
                # Auth/session errors: likely session expired — trigger re-login
